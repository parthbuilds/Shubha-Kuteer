...
const listPagination = document.querySelector(".list-pagination");
const productViewItem = document.querySelector(".product-view-item");
const countProductShow = document.querySelector(".count-product-show");
...
const filterOptionTitles = document.querySelectorAll(".filter-option .title");
const listColors = document.querySelectorAll(".list-color");
const listProduct = document.querySelector('.list-product');
const countInList = document.querySelector('.count-in-list');


// handle layout cols in list product
const productLayoutBtns = document.querySelectorAll('.product-layout-btn')
productLayoutBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        const productLayoutBtnActive = document.querySelector('.product-layout-btn.active')
        productLayoutBtnActive.classList.remove('active')
        btn.classList.add('active')
        const dataCols = btn.getAttribute('data-cols')
        listProduct.classList.remove('cols-2', 'cols-3', 'cols-4', 'cols-5')
        listProduct.classList.add(`cols-${dataCols}`)
    })
})
...
const getFilteredProducts = (products, filterOptions) => {
    return products.filter(product => {
        ...
        // Filter by brand
        if (filterOptions.brand.length > 0 && !filterOptions.brand.includes(product.brand)) {
            isMatch = false;
        }
        ...
        if (filterOptions.sizes.length > 0) {
            let hasSize = false;
            for (const size of filterOptions.sizes) {
        ...
        if (filterOptions.colors.length > 0) {
            ...
            for (const color of filterOptions.colors) {
                if (product.variation.some(v => v.color === color)) {
                    hasColor = true;
                    break;
                }
            }
        ...
        if (product.price < filterOptions.price.min * 100 || product.price > filterOptions.price.max * 100) {
            isMatch = false;
        }
...
const handleFilterChange = () => {
    // Clear and reset state as needed
};
...
// ----- Rerender product base on items filtered -----
const rerenderProducts = (products) => {
    const filteredProducts = filterProducts(products);
    const sortedProducts = sortProducts(filteredProducts, sortBy.value);
    currentPage = 1;
    renderProducts(currentPage, sortedProducts);
    renderPagination(sortedProducts);
    if (countInList) {
        countInList.textContent = sortedProducts.length;
    }
};
...
const handleFilter = (filterType, value) => {
    if (filterType === 'price') {
        ...
        if (filterOptions[filterType].includes(value)) {
            filterOptions[filterType] = filterOptions[filterType].filter(item => item !== value);
        } else {
            filterOptions[filterType].push(value);
        }
    }
    fetchProducts();
};
...
if (filterOptionTitles.length > 0) {
    filterOptionTitles.forEach(title => {
        title.addEventListener('click', () => {
            const parent = title.closest('.filter-option-item');
...
const createProductItem = (product) => {
    const productItem = document.createElement("div");
    productItem.classList.add("col-item");
    productItem.innerHTML = `
        <div class="product-item">
            ...
            <div class="content">
                <a href="product-detail.html?id=${product.id}" class="cate">${product.category}</a>
                <a href="product-detail.html?id=${product.id}" class="title">${product.name}</a>
                <div class="stars">
                    <i class="icon-star"></i>
                    <i class="icon-star"></i>
                    <i class="icon-star"></i>
                ...
                <div class="product-price">
                    ...
                    <p class="origin-price">$${(product.originPrice / 100).toFixed(2)}</p>
                </div>
            </div>
            <div class="product-item-action">
                <button class="add-to-cart-btn" data-id="${product.id}" data-action="add-to-cart">
                    <i class="icon-shopping-cart"></i>
                    <span>Add to cart</span>
                </button>
                <div class="action-btn">
                    <button class="add-to-wishlist-btn" data-id="${product.id}" data-action="add-to-wishlist"><i class="icon-heart"></i></button>
                    <button class="add-to-compare-btn" data-id="${product.id}" data-action="add-to-compare"><i class="icon-shuffle"></i></button>
                    <button class="add-to-quickview-btn" data-id="${product.id}" data-action="add-to-quickview"><i class="icon-search-plus"></i></button>
                </div>
            </div>
        </div>
    `;
...
/**
 * Transforms a single product object from the backend API format
 * to the frontend-friendly format your code expects.
 * @param {object} backendProduct - The product object from the backend.
 * @returns {object} The formatted product object for the frontend.
 */
const formatProductForFrontend = (backendProduct) => {
    // Helper function to safely parse the gallery string
    ...
    // The backend `gallery` field needs to be parsed
    const gallery = parseJsonString(backendProduct.gallery);
    // The sizes field needs to be parsed from a JSON string
    const sizes = parseJsonString(backendProduct.sizes);
    // The variations field needs to be parsed from a JSON string
    const variations = parseJsonString(backendProduct.variations);
    ...
    return {
        ...
        // Convert tinyint(1) to booleans
        new: backendProduct.is_new === 1,
        sale: backendProduct.on_sale === 1,
        // Convert decimal strings to numbers
        rate: parseFloat(backendProduct.rate),
        price: parseFloat(backendProduct.price) * 100, // Convert to cents
        originPrice: parseFloat(backendProduct.origin_price) * 100, // Convert to cents
        brand: backendProduct.brand,
        sold: backendProduct.sold,
        quantity: backendProduct.quantity,
        // Use the value from the database if available, otherwise default to 1
        quantityPurchase: backendProduct.quantity_purchase || 1, 
        sizes: Array.isArray(sizes) ? sizes : [], // Ensure sizes is an array
        variation: Array.isArray(variations) ? variations : [], // Ensure variations is an array
        // Use thumb_image and main_image from the database
        thumbImage: [backendProduct.thumb_image, backendProduct.main_image],
        images: Array.isArray(gallery) ? gallery : [], // Ensure images is an array
...
const fetchProducts = () => {
    ...
    if (productList) {
        productList.innerHTML = `<p class="text-center w-full">Loading products...</p>`;
    }
    
    // Change this fetch call to your backend API
    fetch('/api/admin/products')
        ...
        .then(backendProducts => {
            // Apply the transformation function to each product
            const products = backendProducts.map(formatProductForFrontend);
            const filteredProducts = filterProducts(products);
            const sortedProducts = sortProducts(filteredProducts, sortBy.value);
            
            renderProducts(currentPage, sortedProducts);
            renderPagination(sortedProducts);
            addEventToProductItem(sortedProducts);

            if (totalProduct && countProductShow) {
                totalProduct.textContent = sortedProducts.length;
                countProductShow.textContent = sortedProducts.length > itemsPerPage ? itemsPerPage : sortedProducts.length;
            }
        })
        .catch(error => {
            console.error("There was a problem with the fetch operation:", error);
            if (productList) {
                productList.innerHTML = `<p class="text-red-500 text-center w-full">Failed to load products. Please try again later.</p>`;
            }
        });
};
...
const renderProducts = (page, products) => {
    ...
    if (countProductShow) {
        countProductShow.textContent = productsToRender.length;
    }
};
