    ...
    try {
        ...
        layoutItems.forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                // When layout changes, re-render with the currently filtered set of products
                // We need to know what the *current* filtered state is to re-render.
                // For now, let's assume `window.productsData` holds the currently filtered (or all) products.
                window.renderProducts(1, window.productsData);
                currentPage = 1;
                window.renderPagination(window.productsData)
                window.addEventToProductItem(window.productsData) // Re-attach events after layout change
            })
        })
