...
// ✅ Handle login and save user data locally
...
// ✅ Example usage (attach to your form)
...
function showUserDetailsFromLocalStorage() {
    ...
    // Inject name and email into the HTML
    const nameEl = document.querySelector(".name");
    const mailEl = document.querySelector(".mail");
    const contactName = document.querySelector(".order_name");
    const contactEmail = document.querySelector(".order_email");

    if (nameEl) nameEl.textContent = fullName || "User";
    if (mailEl) mailEl.textContent = email || "No Email";
    if (contactName) contactName.textContent = fullName || "User";
    if (contactEmail) contactEmail.textContent = email || "No Email";

    console.log(`User loaded: ${fullName} (${email})`);
}
...
async function fetchDashboardData() {
    try {
        ...
        if (response.ok && data.userData) {
            ...
            // Update UI with new info if needed
        } else {
            console.warn("Failed to fetch dashboard content:", data.message);
        }
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
    showUserDetailsFromLocalStorage();
    fetchDashboardData();
});
