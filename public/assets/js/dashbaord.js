document.addEventListener("DOMContentLoaded", () => {
  ...
  const cancelledOrdersCountEl = document.getElementById(
  ...
  const recentOrdersTableBody = document.querySelector(
    '.filter-item[data-item="dashboard"] .recent_order table tbody'
  );

  // History Orders Tab Container
  const listOrderContainer = document.querySelector(
    ".filter-item.tab_order .list_order"
  );
  ...
  const getOrderStatusDisplay = (order) => {
    ...
    if (order.status === "cancelled") {
      ...
      order.delivery_status === "out_for_delivery_at"
    ) {
  ...
  async function fetchAndRenderUserOrders() {
    if (!storedUserEmail) {
      console.warn(
        "No user email found for fetching orders. User might not be logged in."
      );
      ...
      if (recentOrdersTableBody)
        recentOrdersTableBody.innerHTML =
          '<tr><td colspan="4" class="text-center py-5">Please log in to view recent orders.</td></tr>';
    ...
    try {
      ...
      if (data.success && Array.isArray(data.orders)) {
        ...
        if (userOrders.length > 0) {
          ...
          const awaitingPickup = userOrders.filter(
            (order) =>
              (order.status === "pending" || order.status === "completed") &&
              order.delivery_status !== "delivered_at" &&
              order.delivery_status !== "out for delivery" &&
              order.delivery_status !== "out_for_delivery_at" &&
          ...
          if (awaitingPickupCountEl)
            awaitingPickupCountEl.textContent = awaitingPickup;
          if (cancelledOrdersCountEl)
            cancelledOrdersCountEl.textContent = cancelled;
                      ...
                      (product) => `
                                        <div class="prd_item flex flex-wrap items-center justify-between gap-3 py-5 border-b border-line">
                                            ...
                                            <div class="text-title">
                                                <span class="prd_quantity">${
                                                  product.quantity
                                                }</span>
                                                <span> X </span>
                                                <span class="prd_price">₹${parseFloat(
                                                  product.price
                                                ).toFixed(2)}</span>
                                            </div>
                                        </div>
          ...
          if (recentOrdersTableBody) {
            ...
            if (recentThreeOrders.length === 0) {
              recentOrdersTableBody.innerHTML =
                '<tr><td colspan="4" class="text-center py-5">No recent orders.</td></tr>';
            } else {
              recentThreeOrders.forEach((order) => {
                let mainProduct = {
                  name: "N/A",
                  category: "N/A",
                  image: "/assets/images/product/productDefault.png",
                  id: "",
                };
                if (order.products) {
                  let productsArray = order.products;
                  if (typeof productsArray === "string") {
                    try {
                      ...
                      console.error(
                        `Error parsing products for recent order ${order.id}:`,
                        e
                      );
                      productsArray = [];
                    }
                ...
                recentOrdersTableBody.innerHTML += `
                                    <tr class="item duration-300">
                                            ...
                                            <a href="product-default.html?id=${
                                              mainProduct.id || ""
                                                ...
                                                <div class="info flex flex-col">
                                                    <strong class="product_name text-secondary">${
                                                      mainProduct.name
                                                    }</strong>
                                                    <span class="product_tag caption1 text-secondary">${
                                        ...
                                        <td class="py-3 price">₹${parseFloat(
                                          order.amount
                                        ).toFixed(2)}</td>
        ...
        console.error(
          "API response format error: 'success' flag is false or 'orders' is not an array.",
          data
        );
        if (listOrderContainer)
          listOrderContainer.innerHTML =
            "<p>Failed to process orders from the server.</p>";
      }
    } catch (error) {
  ...
  if (isLoggedIn === "true" && storedUserEmail) {
    ...
    if (listOrderContainer)
      listOrderContainer.innerHTML = "Please log in to view your orders.";
    if (recentOrdersTableBody)
      recentOrdersTableBody.innerHTML =
        '<tr><td colspan="4" class="text-center py-5">Please log in to view recent orders.</td></tr>';
    if (awaitingPickupCountEl) awaitingPickupCountEl.textContent = "0";
    ...
    if (totalOrdersCountEl) totalOrdersCountEl.textContent = "0";
    console.log(
      "User is not logged in or email is not available in localStorage."
    );
  }
  ...
  tabItems.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      document
        .querySelector(`.filter-item[data-item="${targetTab}"]`)
  ...
  const orderTabButtons = document.querySelectorAll(
    ".tab_order .menu-tab .tab-item"
  );
  const orderTabIndicator = document.querySelector(
    ".tab_order .menu-tab .indicator"
  );
  ...
  orderTabButtons.forEach((button, index) => {
    button.addEventListener("click", () => {
      ...
      console.log(
        "Order tab clicked:",
        filterType || button.textContent.trim()
      );
  ...
  const initialActiveOrderTab = document.querySelector(
    ".tab_order .menu-tab .tab-item.active"
  );
